...
def get_structured_data(rawoutput, module):
    """Gets structured data from raw output."""
    ...
    attrs = dict(Command=module.params["command"], Platform=module.params["platform"])
    try:
...
def parse_raw_output(rawoutput, module):
    """Return a dict if using netmiko and list of dicts if using trigger."""
...
def main():  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
    ...
    module = AnsibleModule(
        argument_spec=argument_spec, mutually_exclusive=(["host", "trigger_device_list"],), supports_check_mode=False
    )
    ...
    if connection in ["ssh", "netmiko_ssh", "netmiko_telnet", "telnet"] and not module.params["host"]:
        module.fail_json(msg="specify host when connection=" "ssh/netmiko_ssh/netmiko_telnet")

    if connection in ["netmiko_telnet", "telnet"] and platform != "cisco_ios":
        module.fail_json(msg="only cisco_ios supports " "telnet/netmiko_telnet connection")
    ...
    if connection in ["ssh", "netmiko_ssh", "netmiko_telnet", "telnet"]:
        ...
        if optional_args:
            module.deprecate(msg="optional_args is deprecated in favor of connection_args.")
            kwargs.update(optional_args)
        if connection_args:
            kwargs.update(connection_args)

        if host:
            commando = Commando(devices=[host], commands=[command], creds=(username, password), **kwargs)
            commando.run()
            rawtxt = commando.results[host][command]
        elif trigger_device_list:
            commando = Commando(devices=trigger_device_list, commands=[command], creds=(username, password), **kwargs)
            commando.run()

    elif connection == "offline":
        with open(raw_file, "r") as data:  # pylint: disable=unspecified-encoding
            rawtxt = data.read()

    if local_file:
        with open(local_file, "w") as f:  # pylint: disable=unspecified-encoding
            f.write(rawtxt)
